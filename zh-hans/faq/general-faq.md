<!-- ### 什么情况下订单会失败（failed）？
1. 区块链分叉：在软分叉期间系统连接的节点在弱势链上并检测交易状态，但最终分叉结束时交易其实没被打包在强势链上。
2. 交易上链失败：系统发送的交易被节点抛弃。
3. 智能合约执行失败：该情况在以太坊常见。以太坊交易成功但智能合约执行失败，也就是交易付ETH矿工费成功但并没有转账代币成功。发生该情况可能由于gas不够或者执行违反合约操作（合约报错），例如转账超出余额的金额。
4. 节点因为异常或同步不完全，而找不到交易或告知Hub系统不正确的信息。 -->
...
<!-- ### 前置系统怎么判断是否重发提现请求？
订单回调中有sendAgain字段，如果是true，则建议前置系统重发，反之不建议。目前sendAgain字段有可能是true的区块链有Ethereum、ChainX、Palkadot和Kusama，其他区块链的sendAgain一定是false。我们建议前置系统对提现请求进行重发最多执行一次，如果订单仍然失败，应引入人工审核流程。

### 订单回调sendAgain是true的判断条件是什么？
1. 基本判断：在节点获取交易哈希信息，报错或结果为空（通过扫描区块判断订单状态的区块链没有这层判断，例如EOS）
2. 区块链属性角度判断：通过区块链特有的属性，判断交易失败，例如ETH的nonce机制、EOS的交易过期机制（必须满足）
3. 默认判断：默认逻辑，每个区块链可经过该判断，即交易在连续n个区块数产生的情况下都不能被找到，n可配置（必须满足）
4. 风控判断：即使满足上述三点，如果订单金额过高超过风控设置“重发阈值”，不会推荐自动重发。每个代币都可以修改“重发阈值”。 -->
